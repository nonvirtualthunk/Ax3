...
case class CreaturesLeftSightEvent ( subject_ : CreatureEntity, others : Set[CreatureEntity] ) extends TGameEvent {
	override def subject = Some(subject_)
	override def objects: List[GameEntity] = others.toList
}

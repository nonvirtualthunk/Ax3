...
class GranularMaterialRenderer extends AnthologiconBaseEnvironmentRenderer with TEnvironmentRenderer {
	def attributeProfile = StandardAttributeProfile

	def effectiveFluidLevel(byte: Byte) = byte
	...
	def updateTalea ( vbo: AVBO , textureBlock : TextureBlock , env : World ,wloc: VoxelCoord , offset : ReadVec3i , limit : ReadVec3i ){
		Analytics.timeDataPoint("liquid renderer") {
			try{
				...
				if ( ! granularData.definedAt(wloc) || granularData.levelGrid.taleaForReadOnly(wloc.x,wloc.y,wloc.z).areAll(0.toByte) ) { return }
				...
				val typeWindow = granularData.materialGrid.windowCenteredOnTaleaAt(wloc,readOnly = true)
				val liquidLevelWindow = granularData.levelGrid.windowCenteredOnTaleaAt(wloc,readOnly = true)
				...
				val liquidTypeCenter = typeWindow.centerTalea
				...
				val oneOverMaxFluidLevel = (1.0f / granularData.maxLevel.toFloat) * 0.9f
				...
				for ( x <- offset.x until limit.x optimized ; y <- offset.y until limit.y optimized ; z <- offset.z until limit.z optimized )
				{
					...
					if ( curLevel > 0 ){
						...
						val liquidTypeView = if ( useCenter ) { liquidTypeCenter } else { typeWindow }
...
//														counter += 1.0f
	//												}
													...
													fluidHeights(fi) += math.max(curLevel * oneOverMaxFluidLevel,0.65f)
//													counter += 1.0f
												}
//											}
											fj += 1}
//										fluidHeights(fi) = math.min(fluidHeights(fi) / counter,1.0f)
										fluidHeights(fi) = math.min(fluidHeights(fi) * 0.25f,1.0f)
...
//								if ( maximumFluidHeight > minimumUsableFluidHeight && minimumFluidHeight > minimumUsableFluidHeight) {
									...
									while ( k < 4 ){
										...
										val cornerIndex = if ( cp.x < 0.5f && cp.y < 0.5f ) {
											0
										...
										val fluidZ = fluidHeights(cornerIndex)

										val pcnt = if ( q == Top ) {
											val ak = (k+1)&3
											val bk = (k+3)&3
											val cpa = cubePointsFlat((q << 2) + ak)
											val cpb = cubePointsFlat((q << 2) + bk)
											val A = ReadVec3f(cp.x - cpa.x,cp.y - cpa.y,fluidZ - fluidHeights(ak)).normalize
											val B = ReadVec3f(cp.x - cpb.x,cp.y - cpb.y,fluidZ - fluidHeights(bk)).normalize

											val normal = A.cross(B)

											normal.dot(Vec3f.UnitZ)
										} else { 1.0f }
										...
										vbo.setAbf(GL,v + k, materialColor.r,materialColor.g,materialColor.b,globalLightStrength * sidingMult * pcnt,128)

...
object BillboardAttributeProfile extends AttributeProfile(
	List(
		"Vertex" ->(3, GL_FLOAT),
		"TexCoord" ->(2, GL_FLOAT),
		"LocalLight" ->(4, GL_UNSIGNED_BYTE),
		"GlobalLight" ->(4, GL_UNSIGNED_BYTE),
		"BillboardOffset" -> (2, GL_FLOAT))
) {
	...
	def createPointBuilder() = new PointBuilder(this) {
		...
		val boff = attributes(BillboardOffsetAttribute).byteOffset
